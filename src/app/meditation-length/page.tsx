...
// import { createUser } from '../../lib/database' // Import createUser - disabled due to CSP issues
...
export default function MeditationLengthPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Skip Firebase for now due to CSP issues - use localStorage only
      const firebaseUserId = `user-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`
      localStorage.setItem('userId', firebaseUserId)
      console.log('User created with ID (localStorage only):', firebaseUserId)
